...
test('update poll', async () => {
  const getstr =
    await request(put(`/polls/${pollres.data.node.id}`, opts2, token));
  const getres = JSON.parse(getstr);
  expect(getres.success).toBeTruthy();
...
test('update poll with invalid token', async () => {
  const getstr =
    await request(put(`/polls/${pollres.data.node.id}`, opts3, 'invalid'));
  const getres = JSON.parse(getstr);
...
test('delete poll', async () => {
  const result =
    await request(del(`/polls/${pollres.data.node.id}`, token));
