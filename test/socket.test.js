...
test('End poll', async () => {
  ...
  expect(createdPoll.text).toBe(poll.text);
  expect(createdPoll.correctAnswer).toBe(poll.correctAnswer);
  expect(createdPoll.state).toBe(constants.POLL_STATES.ENDED);
  expect(createdPoll.answerChoices).toEqual(poll.answerChoices);
